import React, { Component } from "react";
import {
  Col,
  Button,
  Form,
  FormGroup,
  Label,
  Input,
  Container,
  Row,
  Media
} from "reactstrap";
import "../../assets/styles/font.css";
import AddWorkerSkill from "./addWorkerSkill";
import SkillComponent from "./skill";

class WorkerProfileComponent extends Component {
  state = {};

  handleEditProfile = () =>{
    let form1 = document.getElementById("form1");
    form1.style.display = "none";
    let form2 = document.getElementById("form2");
    form2.style.display = "block";
  }

  handleUpdateProfile = () => {
    let form2 = document.getElementById("form2");
    form2.style.display = "none";
    let form1 = document.getElementById("form1");
    form1.style.display = "block";
  }
  render() {
    return (
      <Row style={{ marginTop: 50, fontFamily: "Josefin Sans", fontSize: 20 }}>
        <Col
          xs="6"
          sm="2"
          style={{ backgroundColor: "#f5f5f5", marginLeft: 100 }}
        >
          <Media>
            <Media
              style={{ width: 200, height: 200, marginTop: 30, marginLeft: 40 }}
              object
              src="data:image/png;base64,"
              alt="Generic placeholder image"
            />
          </Media>

          <Button color="success" style={{ marginLeft: 50, marginTop: 20 }}>
            Upload New Photo
          </Button>
        </Col>

        <Col
          xs="6"
          sm="5"
          style={{ backgroundColor: "#f5f5f5", marginLeft: 90 }}
        >
          <Form style={{ padding: 80  }} id = "form1" >
            <FormGroup row>
              <Label for="firstName" sm={4}>
                First Name
              </Label>
              <Col sm={6}>
                <Input
                  type="text"
                  name="firstname"
                  id="firstname"
                  placeholder="first name" disabled
          
                />
              </Col>
            </FormGroup>
            <FormGroup row>
              <Label for="lastName" sm={4}>
                Last Name
              </Label>
              <Col sm={6}>
                <Input
                  type="text"
                  name="lastname"
                  id="lastname"
                  placeholder="last name" disabled
                />
              </Col>
            </FormGroup>

            <FormGroup row>
              <Label for="contactno" sm={4}>
                Phone Number
              </Label>
              <Col sm={6}>
                <Input
                  type="text"
                  name="contactnumber"
                  id="contactnumber"
                  placeholder="phone number"  disabled
                />
              </Col>
            </FormGroup>
            <FormGroup row>
              <Label for="exampleSelect" sm={4}>
                Base Location
              </Label>
              <Col sm={6}>
                <Input type="select" name="select" id="exampleSelect"  disabled>
                  <option>Kollupitiya</option>
                  <option>Dehiwala</option>
                  {/* select base locations from db */}
                </Input>
              </Col>
            </FormGroup>

            <FormGroup row>
              <Label sm={4}>Status</Label>

              <Col sm={6}>
                <Input type="select" name="select" id="exampleSelect"  disabled>
                  <option>Online</option>
                  <option>Offiline</option>
                </Input>
              </Col>
            </FormGroup>

            <FormGroup check row>
              <Col sm={{ size: 30 , marginTop : 10 }}>
                <Button color="success" onClick = {this.handleEditProfile}>Edit Profile</Button>
              </Col>
            </FormGroup>
          </Form>

          <Form style={{ padding: 80 , display : "none" }} id = "form2" >
            <FormGroup row>
              <Label for="firstName" sm={4}>
                First Name
              </Label>
              <Col sm={6}>
                <Input
                  type="text"
                  name="firstname"
                  id="firstname"
                  placeholder="first name"
                />
              </Col>
            </FormGroup>
            <FormGroup row>
              <Label for="lastName" sm={4}>
                Last Name
              </Label>
              <Col sm={6}>
                <Input
                  type="text"
                  name="lastname"
                  id="lastname"
                  placeholder="last name"
                />
              </Col>
            </FormGroup>

            <FormGroup row>
              <Label for="contactno" sm={4}>
                Phone Number
              </Label>
              <Col sm={6}>
                <Input
                  type="text"
                  name="contactnumber"
                  id="contactnumber"
                  placeholder="phone number"
                />
              </Col>
            </FormGroup>
            <FormGroup row>
              <Label for="exampleSelect" sm={4}>
                Base Location
              </Label>
              <Col sm={6}>
                <Input type="select" name="select" id="exampleSelect">
                  <option>Kollupitiya</option>
                  <option>Dehiwala</option>
                  {/* select base locations from db */}
                </Input>
              </Col>
            </FormGroup>

            <FormGroup row>
              <Label sm={4}>Status</Label>

              <Col sm={6}>
                <Input type="select" name="select" id="exampleSelect">
                  <option>Online</option>
                  <option>Offiline</option>
                </Input>
              </Col>
            </FormGroup>

            <FormGroup check row>
              <Col  style = {{marginTop : 10}}>
                <Button color="success" onClick = {this.handleUpdateProfile} style = {{width : 200}}>Update Profile</Button>
                <Button color="warning" onClick = {this.handleUpdateProfile} style = {{marginLeft : 180, width : 200}}>Cancel</Button>
              </Col>
            </FormGroup>
          </Form>
        </Col>

        <Col 
          xs="6"
          sm="3"
          style={{ backgroundColor: "#f5f5f5", marginLeft: 90 ,overflowY :"scroll" , height : 500 }}
        >
          <AddWorkerSkill />
          <SkillComponent/>
          <SkillComponent/>
        </Col>
      </Row>
    );
  }
}

export default WorkerProfileComponent;
